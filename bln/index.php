	...
	switch($_GET['ajax']) {
		case 'entries':
				if(isset($_GET['tagid']) && +$_GET['tagid']) {
					$entries = 'select e.id, e.url, e.posted, e.title, left(e.content, locate(\'</p>\', e.content) + 3) as content, count(c.entry) as comments from blog_entrytags as et left join blog_entries as e on e.id=et.entry left join blog_comments as c on c.entry=e.id where et.tag=\'' . +$_GET['tagid'] . '\' and e.status=\'published\'';
					...
					$entries = 'select e.id, e.url, e.posted, e.title, left(e.content, locate(\'</p>\', e.content) + 3) as content, count(c.entry) as comments from blog_entries as e left join blog_comments as c on c.entry=e.id where status=\'published\'';
					if(isset($_GET['before']) && +$_GET['before'])
						$entries .= ' and e.posted<\'' . +$_GET['before'] . '\'';
					$entries .= ' group by e.id order by e.posted desc limit ' . NUM_ENTRIES;
				}
...
?>
			...
			<article>
				...
				<footer>
					<p class=readmore>
						<a class=continue data-bind="attr: {href: url, title: 'read the rest of this entry'}">continue reading</a>
						<a class=comments data-bind="text: comments + ' comments', attr: {href: url + '#comments', title: (comments ? 'join' : 'start') + ' the discussion on this entry'}"></a>
					</p>
				</footer>
			</article>
