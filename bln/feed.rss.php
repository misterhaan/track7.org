...
if(isset($_GET['tags']) && $_GET['tags']) {
	$rss = new t7feed('track7 blog entries', '/bln/', 'blog entries posted at track7 tagged with ' . $_GET['tags'], 'copyright 2008 - 2017 track7');
...
if($entries = $db->query($entries))
	while($entry = $entries->fetch_object()) {
		...
		$entry->content = str_replace('href="/', 'href="' . t7format::FullUrl('/'), $entry->content);
