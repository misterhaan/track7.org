<?php
  ...
  $act = $update = $forum = $comment = $guide = $art = $round = false;
  ...
  while($items < MAXITEMS && ($act || $update || $forum || $comment || $photo || $guide || $art || $round)) {
    if($act && (!$update || $act->posted > $update->posted) && (!$forum || $act->posted > $forum->posted) && (!$comment || $act->posted > $comment->posted) && (!$guide || $act->posted > $guide->posted) && (!$art || $act->posted > $art->posted) && (!$round || $act->posted > $round->posted)) {
      if($act->hasmore)
      ...
      $act = $acts->fetch_object();
    } elseif($update && (!$forum || $update->posted > $forum->posted) && (!$comment || $update->posted > $comment->posted) && (!$guide || $update->posted > $guide->posted) && (!$art || $update->posted > $art->posted) && (!$round || $update->posted > $round->posted)) {
      ...
      $update = $updates->fetch_object();
    } elseif($forum && (!$comment || $forum->posted > $comment->posted) && (!$guide || $forum->posted > $guide->posted) && (!$art || $forum->posted > $art->posted) && (!$round || $forum->posted > $round->posted)) {
      ...
      $forum = $forums->fetch_object();
    } elseif($comment && (!$guide || $comment->posted > $guide->posted) && (!$round || $comment->posted > $round->posted)) {
