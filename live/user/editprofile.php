    ...
    switch($_GET['tab']) {
      ...
      case 'display':
        if($_POST['submit'] == 'update' && is_numeric($_POST['style'])) {
          ...
          $update = 'update users set tzoffset=' . $update . ', flags=flags' . (isset($_POST['dst']) ? '|' . _FLAG_USERS_DST : '&' . (_FLAG_USERS ^ _FLAG_USERS_DST)) . ', style=' . ($_POST['style']) . ' where uid=' . $u->uid;
          if(false !== $db->Change($update, 'error updating display preferences'))
            $page->Info('display preferences successfully updated');
        } else
          $_POST['style'] = $u->style;
        break;
      case 'contact':
    ...
    switch($_GET['tab']) {
      ...
      case 'display':
        ...
        $prof->AddHTML('style', '<table cellspacing="0" id="colorchoice">'
                       ...
                       . "\n" . '                <td><label for="fldstylefire"><img src="/style/fire/thumb.png" alt="" /><input type="radio" id="fldstylefire" name="style" value="3" ' . ($_POST['style'] == 3 ? 'checked="checked" ' : '') . '/>fire</label></td>'
                       . "\n" . '              </tr><tr>'
                       . "\n" . '                <td><label for="fldstyleearth"><img src="/style/earth/thumb.png" /><input type="radio" id="fldstyleearth" name="style" value="5" ' . ($_POST['style'] == 5 ? 'checked="checked" ' : '') . '/>earth</label></td>'
                       . "\n" . '                <td><label for="fldstyleair"><img src="/style/air/thumb.png" /><input type="radio" id="fldstyleair" name="style" value="7" ' . ($_POST['style'] == 7 ? 'checked="checked" ' : '') . '/>air</label></td>'
                       . "\n" . '              </tr>'
                       . "\n" . '            </table>');
        break;
      case 'contact':
