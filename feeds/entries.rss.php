<?
  ...
  if($_GET['tags'])
    if(substr($_GET['tags'], 0, 1) == '-') {
      $rss = new auFeed('bln entries', '/bln/', 'entries posted on bln at track7 not tagged with ' . substr($_GET['tags'], 1), 'copyright 2008 - 2011 track7');
      ...
      $rss = new auFeed('bln entries', '/bln/', 'entries posted on bln at track7 tagged with ' . $_GET['tags'], 'copyright 2008 - 2011 track7');
    ...
    $rss = new auFeed('all bln entries', '/bln/', 'all entries posted on bln at track7', 'copyright 2008 - 2011 track7');
  ...
  if($entries = $db->GetLimit($entries, 0, 15, '', ''))
    while($entry = $entries->NextRecord()) {
      ...
      $rss->AddItem($tags . $entry->post . '<p>» <a href="/bln/' . $entry->id . '">read more...</a></p>', $entry->title, '/bln/' . $entry->name, $entry->instant, '/bln/' . $entry->name, true);
    }
  $rss->End();
?>
