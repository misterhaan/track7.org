<?
  ...
  if($_GET['tags'])
    if(substr($_GET['tags'], 0, 1) == '-') {
      $rss = new auFeed('guides', '/guides/', 'guides posted on track7 not tagged with ' . substr($_GET['tags'], 1), 'copyright 2008 - 2011 track7');
      ...
      $rss = new auFeed('guides', '/guides/', 'guides posted on track7 tagged with ' . $_GET['tags'], 'copyright 2008 - 2011 track7');
    ...
    $rss = new auFeed('guides', '/guides/', 'all guides posted at track7', 'copyright 2008 - 2011 track7');
  ...
  if($guides = $db->GetLimit($guides, 0, 15, '', ''))
    while($guide = $guides->NextRecord()) {
      ...
      $rss->AddItem('<p>' . $guide->description . '</p>', $guide->title, '/guides/' . $guide->id . '/', $guide->dateadded, '/guides/' . $guide->id . '/', true);
    }
  $rss->End();
?>
