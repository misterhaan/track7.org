<?
  ...
  $rss = new auFeed('track7 disc golf rounds', '/discgolf/rounds.php', 'disc golf rounds posted on track7', 'copyright 2011 track7');
  ...
  if($rounds = $db->GetLimit($rounds, 0, 15, '', ''))
    while($round = $rounds->NextRecord()) {
      $rss->AddItem('<p>' . ($round->uid ? '<a href="http://' . $_SERVER['HTTP_HOST'] . '/discgolf/players.php?p=' . $round->login . '" title="more information on this player">' . $round->login . '</a>' : $round->player) . ' played a ' . $round->roundtype . ' round ' . ($round->tees ? 'from the ' . $round->tees . ' tees ' : '') . 'at <a href="http://' . $_SERVER['HTTP_HOST'] . '/discgolf/courses.php?id=' . $round->courseid . '" title="more information on this course">' . $round->name . '</a>, scoring ' . $round->score . '.</p><p>' . $round->comments . '</p>', $round->name . ' round - ' . ($round->uid ? $round->login : $round->player), '/discgolf/rounds.php?id=' . $round->id, $round->instant, '/discgolf/rounds.php?id=' . $round->id, true);
    }
  $rss->End();
?>
