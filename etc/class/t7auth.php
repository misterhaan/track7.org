                ...
                if($auth->GetUserInfo())
                  if($user->IsLoggedIn()) // link new account to user
                    if($db->real_query('insert into login_' . $auth::SOURCE . ' (user, ' . $auth::FIELD . ', profile) values (\'' . +$user->ID . '\', \'' . $db->escape_string($auth->ID) . '\', \'' . $db->escape_string($auth->ProfileFull) . '\')')) {
                      ...
                      header('Location: http://' . $_SERVER['HTTP_HOST'] . $auth->Continue);
                      die;
  ...
  class t7authGoogle extends t7authRegisterable {
    ...
    public static function GetAuthURL($continue, $csrf) {
      return self::REQUEST . '?' . http_build_query(array(
        ...
        'redirect_uri' => 'http://' . $_SERVER['HTTP_HOST'] . self::REDIRECT,
        'response_type' => 'code',
        'scope' => self::SCOPE,
    ...
    private function GetTokens($code) {
      ...
      curl_setopt($c, CURLOPT_USERAGENT, $_SERVER['HTTP_HOST']);
