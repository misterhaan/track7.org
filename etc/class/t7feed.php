  ...
  class t7feed {
    ...
    public function t7feed($title, $url = '', $description = '', $copyright = '', $lang = 'en-us') {
      ...
      $this->xml->startElement('rss');
        ...
        $this->xml->startElement('channel');
          ...
          if(substr($url, 0, 1) != '/')
            $url = '/' . $url;
          $this->xml->writeElement('link', 'http://' . $_SERVER['HTTP_HOST'] . $url);
          $this->xml->startElement('atom:link');
            $this->xml->writeAttribute('href', 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']);
    ...
    public function AddItem($description = '', $title = '', $url = '', $date = '', $id = '', $idLink = false) {
          $this->xml->startElement('item');
            ...
            if($id)
              if($idLink)
                $this->xml->writeElement('guid', 'http://' . $_SERVER['HTTP_HOST'] . $id);
              else {
