...
class t7contrib {
	...
	public static function GetAll($before = false, $limit = 9) {
		...
		$sql = 'select * from ('
			...
			. 'select case p.subsite when \'album\' then \'photo\' when \'bln\' then \'post\' else p.subsite end as conttype, unix_timestamp(p.instant) as posted, p.url, u.username, u.displayname, \'\' as authorname, \'\' as authorurl, p.title, p.preview, p.hasmore from post as p left join user as u on u.id=p.author where p.published=true'
			. ') as allcontributions';
		if ($before !== false)
	...
	public static function GetUser($userid, $before = false, $limit = 12) {
		...
		$sql = 'select * from ('
			...
			. 'select case p.subsite when \'album\' then \'photo\' when \'bln\' then \'post\' else p.subsite end as conttype, unix_timestamp(p.instant) as posted, p.url, p.title from post as p where p.published=true and p.author=\'' . +$userid . '\''
			. ') as allcontributions';
		if ($before !== false)
			$sql .= ' where posted<' . +$before;
		$sql .=	' order by posted desc limit ' . +$limit;
