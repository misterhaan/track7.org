...
if mysqlshow -h $MYSQL_HOST --user=$MYSQL_USER --password=$MYSQL_PASS | grep -q $MYSQL_NAME; then
	mysqldump -h $MYSQL_HOST -u $MYSQL_USER --password="$MYSQL_PASS" --databases $MYSQL_NAME --routines | perl -pe 's/\!\d+ DEFINER/DEFINER/' | bzip2 -c > $BACKUP_DIR/track7content.sql.bz2
fi
