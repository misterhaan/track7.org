...
class PhotoApi extends Api {
	...
	public static function GetEndpointDocumentation(): array {
		...
		$endpoints[] = $endpoint = new EndpointDocumentation('POST', 'idAvailable', 'checks if a photo id is available.  this means not in use or already used by the specified photo.', 'plain text', 'send the proposed new id for the photo as the request body.');
		$endpoint->PathParameters[] = new ParameterDocumentation('oldId', 'string', 'current id of the photo that might be changing its id.');
