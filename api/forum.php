...
<?php
	...
	protected static function listAction($ajax) {
		...
		$select = 'select d.id, d.title, group_concat(t.name order by t.name) as tags, fr.posted as started, fu.username as startusername, fu.displayname as startuserdisplay, fr.contacturl as startcontact, fr.name as startname, (select count(1) from forum_replies where discussion=d.id)-1 as replies, lr.posted as replied, lu.username as lastusername, lu.displayname as lastuserdisplay, lr.contacturl as lastcontact, lr.name as lastname from '
			...
			. 'group by d.id order by lr.posted desc limit ' . self::MAX_THREADS;
