    ...
    if(!$error) {
      ...
      elseif($upload['found'] && !$upload['saved'])
        ...
        if($videoedit->Submitted() == 'edit') {
          ...
          $ins = 'insert into photos (id, youtubeid, caption, description, taken, tags, added) values (\'' . addslashes($id) . '\', \'' . addslashes(htmlspecialchars($_POST['youtubeid'])) . '\', \'' . addslashes(htmlspecialchars($_POST['caption'], ENT_COMPAT, _CHARSET)) . '\', \'' . addslashes(auText::BB2HTML($_POST['desc'])) . '\', \'' . $taken . '\', \'' . addslashes(htmlspecialchars($_POST['tags'], ENT_COMPAT, _CHARSET)) . '\', \'' . time() . '\')';
          if(false === $db->Put($ins, 'error adding video information'))
            $error = true;
          else {
            $twurl = ' video: ' . auSend::Bitly('http://' . str_replace('m.', 'www.', $_SERVER['HTTP_HOST']) . dirname($_SERVER['PHP_SELF']) . '/photo/' . $id);
