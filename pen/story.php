    ...
    if($story = $story->fetch_object())
      if($story->published || $user->IsAdmin()) {
        if($story->seriesurl && (!isset($_GET['series']) || $_GET['series'] != $story->seriesurl)) {
          header('Location: http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/' . $story->seriesurl . '/' . $story->url);
          die;
        }
        if(!$story->seriesurl && isset($_GET['series'])) {
          header('Location: http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/' . $story->url);
          die;
        }
